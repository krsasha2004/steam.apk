    ...
    public static String translateError(String errMessage) {
        ...
        if (errMessage.startsWith("net::ERR_")) {
            if (Arrays.binarySearch(s_likelyNetworkErrors, errMessage.substring("net::ERR_".length())) >= 0) {
                result = SteamCommunityApplication.GetInstance().getResources().getString(C0151R.string.network_error);
            }
        }
        if (result == null) {
            return SteamCommunityApplication.GetInstance().getResources().getString(C0151R.string.unspecified_error);
        }
        return result;
    }
}
