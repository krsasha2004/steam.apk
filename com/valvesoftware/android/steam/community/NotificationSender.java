...
public class NotificationSender {
    ...
    private HashSet<String> chatPartnerNamesWithRecentlyMadeNotifications;
    private Context context;
    private ChatNotification lastChatNotification;
    private long lastRingOrVibrateTime;
    private SettingInfoDB settingInfoDB;
    private SteamCommunityApplication steamCommunityApplication;

    private NotificationSender() {
        this.settingInfoDB = SteamCommunityApplication.GetInstance().GetSettingInfoDB();
        this.lastRingOrVibrateTime = 0;
        this.chatPartnerNamesWithRecentlyMadeNotifications = new HashSet();
        this.steamCommunityApplication = SteamCommunityApplication.GetInstance();
        this.context = this.steamCommunityApplication.getApplicationContext();
    }
    ...
    public void sendConfirmationNotification(String alert) {
        String str = alert;
        sendNotification(SteamCommunityApplication.GetInstance().getResources().getString(C0151R.string.ConfirmationTitle), str, PendingIntent.getActivity(this.context, 4, SteamAppIntents.viewConfirmations(this.context), 0), 4, shouldRingForGenericNotification(), shouldVibrateForGenericNotification());
    }
    ...
    public void sendChatNotification(String message, String from, boolean isTruncated) {
        ...
        if (this.lastChatNotification != null) {
            if (this.lastChatNotification.matches(from, message, System.currentTimeMillis())) {
                return;
            }
        }
        ...
        if (notificationsBySender != null && notificationsBySender.keySet().size() > 1) {
            ...
            from = this.context.getResources().getString(C0151R.string.Chats);
        ...
        sendNotificationWithHeadsUp(from, message, PendingIntent.getActivity(this.context, 1, chatIntent, 134217728), 1, shouldRing, shouldVibrate, this.context.getResources().getString(C0151R.string.notification_chat));
    }
    ...
    private void sendNotificationWithHeadsUp(String title, String message, PendingIntent resultPendingIntent, int notificationId, boolean ring, boolean vibrate, String publicMessage) {
        ...
        Builder notificationBuilder = new Builder(this.context).setSmallIcon(C0151R.drawable.notification_chat).setContentTitle(title).setContentText(message).setContentIntent(resultPendingIntent).setCategory("msg").setVisibility(1).setPriority(1);
        if (publicMessage != null) {
            notificationBuilder.setContentTitle(this.context.getResources().getString(C0151R.string.app_name)).setContentText(publicMessage).setPublicVersion(notificationBuilder.build()).setVisibility(0).setContentTitle(title).setContentText(message);
        }
    ...
    private void sendNotification(String title, String message, PendingIntent resultPendingIntent, int notificationId, boolean ring, boolean vibrate) {
        ...
        Notification notification = new Builder(this.context).setSmallIcon(C0151R.drawable.notification_chat).setContentTitle(title).setContentText(message).setContentIntent(resultPendingIntent).build();
        notification.flags |= 16;
        if (ring) {
            notification.sound = getRingToneUri();
        }
